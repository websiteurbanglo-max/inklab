...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    console.log(`[webhook:app/scopes_update] START topic=${topic} shop=${shop}`);

    const current = payload.current as string[];
    console.log(`[webhook:app/scopes_update] New scopes: ${current.join(",")}`);

    if (session) {
      console.log(`[webhook:app/scopes_update] Updating session scope in Prisma sessionId=${session.id}`);
      ...
      console.warn(`[webhook:app/scopes_update] No session found — scope not persisted`);
    }
