...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (session) {
      console.log(`[webhook:app/uninstalled] Deleting Prisma sessions for shop=${shop}`);
      await db.session.deleteMany({ where: { shop } });
      console.log(`[webhook:app/uninstalled] Prisma sessions deleted`);
    } else {
      console.log(`[webhook:app/uninstalled] No active session found — skipping session delete`);
    }

    console.log(`[webhook:app/uninstalled] Deactivating shop in Firestore shop=${shop}`);
